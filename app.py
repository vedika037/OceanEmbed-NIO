...
metric_row([
    ...
    ("OHC proxy",f"{ohcc:,.0f} MJ/m²","Integrated heat-content proxy from the prototype profile"),
    ("Mean uncertainty",f"±{mean_unc:.2f} °C","Mean emulator uncertainty across target depths"),
    ("Physics score",f"{physics['score']:.1f}/100","Prototype diagnostic score"),
    ("Confidence",f"{confidence:.0f}%","Derived prototype confidence indicator"),
])
...
with T[0]:
    ...
    with right:
        ...
        status_box(f"Current assessment: <b>{risk}</b>. This is a prototype diagnostic, not an operational ocean forecast.","warning" if risk!="Stable" else "good")
...
with T[3]:
    ...
    st.markdown('<div class="warning">The validation panel is synthetic in this prototype. Replace it with independent INCOIS LAS Gridded ARGO observations for the SIH submission evidence.</div>',unsafe_allow_html=True)
